...
// Debug session to check its contents
var_dump($_SESSION); // This will show you the current session data

header("Cache-Control: no-store, no-cache, must-revalidate, max-age=0");
header("Pragma: no-cache");
header("Expires: 0");

// Check if user is logged in by verifying the 'user_id' in session
if (!isset($_SESSION['a_id'])) {
    // Redirect to login if the session 'user_id' is not set
...
$profilePicture = !empty($userInfo['profile_picture']) ? $userInfo['profile_picture'] : '../img/defaultpfp.png';
...
<body class="sb-nav-fixed bg-dark">
    <nav class="sb-topnav navbar navbar-expand navbar-dark bg-secondary border border-light">
        ...
        <span class="big text-light mb-1">
           <?php
if ($userInfo) {
    // Output the name if $userInfo is not null
    echo htmlspecialchars($userInfo['firstname'] . ' ' . $userInfo['middlename'] . ' ' . $userInfo['lastname']);
} else {
...
if ($userInfo) {
    // Output the name if $userInfo is not null
    echo htmlspecialchars($userInfo['role']);
} else {
    // Handle the case where $userInfo is null
    echo "User information not available.";
}
        ...
        <div id="layoutSidenav_nav">
            <nav class="sb-sidenav accordion sb-sidenav-dark" id="sidenavAccordion">
                <div class="sb-sidenav-menu ">
                    <div class="nav">
                        ...
                        <div class="collapse" id="collapseLM" aria-labelledby="headingOne" data-bs-parent="#sidenavAccordion">
                            <nav class="sb-sidenav-menu-nested nav">
                                <a class="nav-link" href="leave.php">Leave Request</a>
                            </nav>
                        </div>
                ...
                <div class="sb-sidenav-footer">
                    <div class="small">Logged in as: <?php echo htmlspecialchars($userInfo['firstname'] . ' ' . $userInfo['lastname']); ?></div>
                </div>
            </nav>
        </div>
